...
import { ArrowRight, ArrowLeft, FileSpreadsheet, Loader2, AlertCircle, Terminal, Trash2 } from "lucide-react";
...
class BillingErrorBoundary extends Component<{ children: ReactNode }> {
      ...
      return (
        <div className="flex items-center justify-center min-h-[400px] p-8">
          <div className="max-w-lg rounded-lg border border-destructive/50 bg-destructive/5 p-6">
            <h2 className="text-lg font-semibold text-destructive mb-2">Billing page crashed</h2>
            <pre className="text-sm text-muted-foreground whitespace-pre-wrap font-mono">{this.state.error?.message}</pre>
            <button onClick={() => this.setState({ hasError: false, error: null })} className="mt-4 inline-flex items-center justify-center rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90">Try again</button>
          </div>
        </div>
      );
    }
...
function WorkflowBillingPage() {
  ...
  async function fetchData() {
    try {
      const res = await fetch(`${API}/api/extracted/billing`);
  ...
  const hasRecords = extractedRecords.length > 0;
  ...
  const handleDeleteEntries = async () => {
    ...
    try {
      await fetch(`${API}/api/extracted/billing`, { method: "DELETE" });
      setExtractedRecords([]);
      await fetchData();
    } catch {} finally {
  ...
  return (
    <div>
      ...
      <PageHeader
        title="Billing Plans"
        description={hasRecords ? "Showing extracted billing plans" : "Upload and extract billing plans"}
        actions={hasRecords ? (
          <div className="flex items-center gap-2">
            <Button variant="destructive" size="sm" onClick={handleDeleteEntries} disabled={deletingEntries}>
              {deletingEntries ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Trash2 className="mr-2 h-4 w-4" />}
              {deletingEntries ? "Deleting..." : "Delete Entries"}
            </Button>
          </div>
        ) : undefined}
      />
      ...
      {hasRecords ? (
        <DataTable<any> data={extractedRecords} columns={columns} searchPlaceholder="Search billing plans..." />
      ) : (
        <Card>
      ...
      <div className="mt-6 flex justify-between">
        <Button variant="outline" onClick={goBack}><ArrowLeft className="mr-2 h-4 w-4" />Previous</Button>
        <Button onClick={() => navigate("/workflow/students")}>Next: Students<ArrowRight className="ml-2 h-4 w-4" /></Button>
      </div>
    </div>
  );
}
